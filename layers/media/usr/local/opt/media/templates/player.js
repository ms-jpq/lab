...
/** @typedef {{action: "cancel" | "restart", position: number} | {action: "retry", error: unknown, position: number, start: number}} AttemptChange */
  ...
  streaming: for (;;) {
    ...
    try {
      for (;;) {
        ...
        const next_start = stream_position(buffered_end() ?? start)
        if (next_start > start) {
          failures.recover()
        }
        start = next_start
